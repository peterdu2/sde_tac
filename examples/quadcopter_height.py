...
class escape_time_solver:
    ...
    def __init__(self, d, M, max_degree, pcs_m, pcs_b, pcs_equality, y0, q_alpha=5, max_opt_iters=5000,
    ...
    def solve(self, mode='Maximize'):
        ...
        start_time = time.time()
        obj = cp.Maximize(self.exit_time_order*self.mj[self.idx_list.index([0, 0, self.exit_time_order-1])]) if mode == 'Maximize' else cp.Minimize(
            self.exit_time_order*self.mj[self.idx_list.index([0, 0, self.exit_time_order-1])])
        print('Objective', obj)
    ...
    def martingale_constraints(self):
        ...
        for n in range(start, end):
            ...
            if dh[0] != 0:
                # generator term: v * df/dh
                dh[1][V_IDX] += 1	# Increment degree of monomial
                if not dh[1] in self.idx_list:
                    invalid_idx = True
                constriant_str += 'dh[0]*'
                constriant_str += 'self.mj[self.idx_list.index(dh[1])]+'
        ...
        for n in range(start, len(self.mj)):
            ...
            if dh[0] != 0:
                ...
                constriant_str += 'self.mj[self.idx_list.index(dh[1])]+'
            if dv_2[0] != 0:
                # generator term: 0.5  * d^2f/dv^2
                if not dv_2[1] in self.idx_list:
                    invalid_idx = True
                constriant_str += '0.5*dv_2[0]*'
...
if __name__ == '__main__':
    ...
    # -h^2 + 1/4 >= 0
    pcs_h = np.zeros(len(idx_list)).tolist()
    pcs_h[idx_list.index([2, 0, 0])] = -1.
    pcs_h[idx_list.index([0, 0, 0])] = 0.25

    # -v^2 + 1 >= 0
    pcs_v = np.zeros(len(idx_list)).tolist()
    pcs_v[idx_list.index([2, 0, 0])] = -1.
    pcs_v[idx_list.index([0, 0, 0])] = 1.

    # -t^2 + Tt >= 0
    pcs_t = np.zeros(len(idx_list)).tolist()
    pcs_t[idx_list.index([0, 0, 2])] = -1.
    pcs_t[idx_list.index([0, 0, 1])] = T

    # -h + 0.5 >= 0
    pcs_h1 = np.zeros(len(idx_list)).tolist()
    pcs_h1[idx_list.index([1, 0, 0])] = -1.
    pcs_h1[idx_list.index([0, 0, 0])] = 0.5
    # h + 0.5 >= 0
    pcs_h2 = np.zeros(len(idx_list)).tolist()
    pcs_h2[idx_list.index([1, 0, 0])] = 1.
    pcs_h2[idx_list.index([0, 0, 0])] = 0.5
    # -v + 5.5 >= 0
    pcs_v1 = np.zeros(len(idx_list)).tolist()
    pcs_v1[idx_list.index([1, 0, 0])] = -1.
    pcs_v1[idx_list.index([0, 0, 0])] = 1.
    # v + 5.5 >= 0
    pcs_v2 = np.zeros(len(idx_list)).tolist()
    pcs_v2[idx_list.index([1, 0, 0])] = 1.
    pcs_v2[idx_list.index([0, 0, 0])] = 1.

    # Localising matrix polynomials
    pcs_m = [pcs_h, pcs_v, pcs_t]
    pcs_b = []
    #pcs_m = [pcs_h1, pcs_h2, pcs_v1, pcs_v2, pcs_t]
    pcs_b = []

    # Localising matrix equality constraints
    # h^2t - (1/4)t - Th^2 + T/4 >= 0
    idx_list = generate_unsorted_idx_list(M=3, d=state_dim)
    idx_list = sorted(idx_list, key=cmp_to_key(index_comparison))
    pcs_equality = np.zeros(len(idx_list)).tolist()
    pcs_equality[idx_list.index([2, 0, 1])] = 1.
    pcs_equality[idx_list.index([0, 0, 1])] = -0.25
    pcs_equality[idx_list.index([2, 0, 0])] = -T
    pcs_equality[idx_list.index([0, 0, 0])] = T/4

    idx_list = generate_unsorted_idx_list(M=5, d=state_dim)
    idx_list = sorted(idx_list, key=cmp_to_key(index_comparison))
    pcs_equality = np.zeros(len(idx_list)).tolist()
    pcs_equality[idx_list.index([2, 2, 1])] = -1.
    pcs_equality[idx_list.index([2, 0, 1])] = 1.
    pcs_equality[idx_list.index([2, 2, 0])] = T
    pcs_equality[idx_list.index([2, 0, 0])] = -T
    pcs_equality[idx_list.index([0, 2, 1])] = 1/4
    pcs_equality[idx_list.index([0, 0, 1])] = -1/4
    pcs_equality[idx_list.index([0, 2, 0])] = -T/4
    pcs_equality[idx_list.index([0, 0, 0])] = T/4
    ...
    for order in range(3):
        ets = escape_time_solver(d=state_dim, M=num_moments, max_degree=max_degree, q_alpha=q_polynomial_deg,
                                ...
                                solver_verbose=True, exit_time_order=order+1, solver='MOSEK')
    ...
    pickle.dump([values, times, max_degree], open('results/quadcopter_height_max.pkl', 'wb'))
