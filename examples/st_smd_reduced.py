...
class escape_time_solver:
    ...
    def __init__(self, d, M, max_degree, pcs_m, pcs_b, pcs_equality, sys_params, y0,
                 scs_eps, scs_scale, q_alpha=5, max_opt_iters=5000,
    ...
    def solve(self, mode='Maximize'):
        ...
        print('Time to create exit measure equality constraints:',
        ...
        if self.solver == 'SCS':
            self.prob.solve(max_iters=self.max_opt_iters, verbose=self.solver_verbose, solver=cp.SCS,
                            acceleration_lookback=self.acc_lookback, eps=self.scs_eps, scale=self.scs_scale)
    ...
    def exit_measure_equality_constraints(self):
        ...
        num_equality_polys = len(self.polynomial_coeffs_equality)

        for q in range(num_equality_polys):
            # Generate list of indexes for polynomial
            idx_list = generate_unsorted_idx_list(
                M=max_poly_degree, d=self.d)
            idx_list = sorted(idx_list, key=cmp_to_key(index_comparison))
            # Find the largest index in the polynomial coeff that isn't 0
            for k in range(len(idx_list)-1, -1, -1):
                if self.polynomial_coeffs_equality[q][k] != 0:
                    break
            ...
            for i in range(lm_dim):
                for j in range(lm_dim):
                    ...
                    for alpha in range(len(idx_list)):
                        # Get moment index by summing alpha index with moment matrix index
                        lm_idx = [sum(x)
                                  for x in zip(idx_list[alpha], mm_idx)]
                        if self.polynomial_coeffs_equality[q][alpha] != 0:
                            # Generate constraint string
                            constriant_str += 'self.polynomial_coeffs_equality[q]['+str(
                                alpha)+']*'
...
if __name__ == '__main__':
    ...
    pcs_x_quad[idx_list.index([0, 0, 0, 0, 0])] = -x_end*x_start
    pcs_x_quad[idx_list.index([1, 0, 0, 0, 0])] = x_end+x_start
    pcs_x_quad[idx_list.index([2, 0, 0, 0, 0])] = -1.
    ...
    pcs_b = [[-i if i != 0 else 0 for i in pcs_x_quad],
    ...
    #pcs_b = [pcs_x_quad]
    pcs_b = []
    pcs_equality = [pcs_x_quad, pcs_t_quad]
    pcs_m = [pcs_x_quad, pcs_t_quad]
    ...
    ets = escape_time_solver(d=state_dim, M=num_moments, q_alpha=q_polynomial_deg,
                             ...
                             max_degree=max_degree, scs_scale=scs_scale,
                             scs_eps=scs_eps,
