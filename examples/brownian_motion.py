...
class escape_time_solver:
    ...
    def create_local_matrix(self):
        ...
        print('LOCAL MATRIX VARS')
        print(self.localising_matrices_mj)
...
if __name__ == '__main__':
    ...
    # -t + T >= 0
    pcs_t2 = np.zeros(len(idx_list)).tolist()
    pcs_t2[idx_list.index([0, 1])] = -1.
    pcs_t2[idx_list.index([0, 0])] = T
    ...
    pcs_equality[idx_list.index([1, 1])] = -1.
    pcs_equality[idx_list.index([2, 0])] = -T
    pcs_equality[idx_list.index([1, 0])] = T
    ...
    for order in range(1):
        ets = escape_time_solver(d=state_dim, M=num_moments, max_degree=max_degree, q_alpha=q_polynomial_deg,
                                ...
                                max_opt_iters=500000, acc_lookback=10,
