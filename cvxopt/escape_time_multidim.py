...
class escape_time_solver:
	...
	def __init__(self, d, M, pcs_m, pcs_b, q_alpha=5, y0=0.2, lm_vis=True, max_opt_iters=5000,
	...
	def solve(self, mode='Maximize'):
		...
		prob.solve(max_iters=self.max_opt_iters, verbose=self.solver_verbose, solver=cp.SCS, 
			       warm_start=self.warm_start, use_indirect=False, acceleration_lookback=self.acc_lookback)
	...
	def martingale_constraints(self):
		...
		for n in range(start,end):
			...
			# dx, dy, dg, dt, dcos(g), dsin(g)
			dx = mono_derivative(f,[0])
			dy = mono_derivative(f,[1])
			dg = mono_derivative(f,[2])
			dt = mono_derivative(f,[3])
			dcosg = mono_derivative(f,[4])
			dsing = mono_derivative(f,[5])
		...
		for n in range(start, len(self.mj)):
			...
			# For each test function perform the following derivatives:
			# dx, dy, dg, dt, dcos(g), dsin(g), dx^2, dxdy, dydx, dy^2, dg^2  
			dx = mono_derivative(f,[0])
			dy = mono_derivative(f,[1])
			dg = mono_derivative(f,[2])
			dt = mono_derivative(f,[3])
...
if __name__ == '__main__':
	...
	# 
	# 2) State space dimensionality: 6
	#    state = [x, y, gamma (g), t, cos(g), sin(g)]
	...
	q_polynomial_deg = 2
	num_inequalities = 1
	idx_list = generate_unsorted_idx_list(M=q_polynomial_deg,d=6)
	idx_list = sorted(idx_list, key=cmp_to_key(index_comparison))
	pcs_m = np.zeros(len(idx_list)).tolist()
	pcs_m[idx_list.index([0,0,0,0,0,0])] = -41
	pcs_m[idx_list.index([1,0,0,0,0,0])] = 10
	pcs_m[idx_list.index([0,1,0,0,0,0])] = 10
	pcs_m[idx_list.index([2,0,0,0,0,0])] = -1
	pcs_m[idx_list.index([0,2,0,0,0,0])] = -1
	...
	# x=0.5, y=0.5, g=0.0, t=0.0, cos(g)=cos(1), sin(g)=sin(1)
	...
	ets = escape_time_solver(d=state_dim, M=moment_degree, q_alpha=q_polynomial_deg, 
		                     ...
		                     max_opt_iters=700000, acc_lookback=10)
	start_time = time.time()
	ets.solve(mode='Maximize')
	print('end time:',time.time()-start_time)

	pickle.dump(ets, open('results/tests/M8_700000_max.pkl', 'wb'))
