	...
	def martingale_constraints(self):
		...
		for n in range(start,end):
			...
			if dx2[0] != 0:
				...
				t3[X1_IDX] += 1
				t3[X2_IDX] += 1
				# Special case with t3 as term involves x1*x2 which may exceed degree limit
				if t3 in self.idx_list:
...
if __name__ == '__main__':
	...
	# idx_list = generate_unsorted_idx_list(M=3,d=2)
	# idx_list = sorted(idx_list, key=cmp_to_key(index_comparison))
	# for i in idx_list:
	# 	print(i)
	# print(len(idx_list), '\n')
	...
	ets = escape_time_solver(d=state_dim, M=moment_degree, q_alpha=q_polynomial_deg, 
		                     ...
		                     max_opt_iters=5000000, acc_lookback=10, sys_params=[ks,ms,kc],
		                     solver_verbose=True)
	...
	#pickle.dump(ets, open('results/SMD/M12_5000000_min.pkl', 'wb'))
