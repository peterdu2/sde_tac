...
class escape_time_solver:
	...
	def __init__(self, d, M, pcs_m, pcs_b_inner, pcs_b_outer, sys_params, warm_start_file_path, q_alpha=5, y0=0.2, lm_vis=True, max_opt_iters=5000,
		...
		self.warm_start_file_path = warm_start_file_path
	...
	def solve(self, mode='Maximize'):
		...
		start_time = time.time()
		obj = cp.Maximize(self.exit_time_order*self.mj[self.idx_list.index([0,0,self.exit_time_order-1])]) if mode == 'Maximize' else cp.Minimize(self.exit_time_order*self.mj[self.idx_list.index([0,0,self.exit_time_order-1])])
		print('Objective', obj)
		...
		if self.solver == 'SCS':
			if self.warm_start:
				# Load the previous optimization data
				ws_data = pickle.load(open(self.warm_start_file_path, 'rb'))
				self.prob = ws_data.prob
				self.prob.solve(max_iters=self.max_opt_iters, verbose=self.solver_verbose, solver=cp.SCS, 
			       				acceleration_lookback=self.acc_lookback, warm_start=True)
			else:
				self.prob.solve(max_iters=self.max_opt_iters, verbose=self.solver_verbose, solver=cp.SCS, 
...
if __name__ == '__main__':
	...
	# x1 >= -2.2
	pcs_x1_lower = np.zeros(len(idx_list)).tolist()
	pcs_x1_lower[idx_list.index([0,0,0])] = 2.2
	pcs_x1_lower[idx_list.index([1,0,0])] = 1.

	# x1 <= -1.0
	pcs_x1_upper = np.zeros(len(idx_list)).tolist()
	pcs_x1_upper[idx_list.index([0,0,0])] = -1.0
	pcs_x1_upper[idx_list.index([1,0,0])] = -1.

	# -2.2 -3.2x1 -x1^2 >= 0
	x1_end=-1.0
	x1_start=-2.3
	pcs_x1_quad = np.zeros(len(idx_list)).tolist()
	pcs_x1_quad[idx_list.index([0,0,0])] = -x1_end*x1_start
	pcs_x1_quad[idx_list.index([1,0,0])] = (x1_end+x1_start)
	pcs_x1_quad[idx_list.index([2,0,0])] = -1.

	# t >= 0
	pcs_t_lower = np.zeros(len(idx_list)).tolist()
	pcs_t_lower[idx_list.index([0,0,0])] = 0.
	pcs_t_lower[idx_list.index([0,0,1])] = 1.

	# t <= 15 
	pcs_t_upper = np.zeros(len(idx_list)).tolist()
	pcs_t_upper[idx_list.index([0,0,0])] = 15.
	pcs_t_upper[idx_list.index([0,0,1])] = -1.

	t_end=15.
	t_start=0.
	pcs_t_quad = np.zeros(len(idx_list)).tolist()
	pcs_t_quad[idx_list.index([0,0,0])] = -t_end*t_start
	pcs_t_quad[idx_list.index([0,0,1])] = (t_end+t_start)
	pcs_t_quad[idx_list.index([0,0,2])] = -1.

	# Linear x1, quadratic t
	pcs_b_inner = [[-i if i != 0 else 0 for i in pcs_x1_lower],
	 			   [-i if i != 0 else 0 for i in pcs_x1_upper],
				   [-i if i != 0 else 0 for i in pcs_t_quad]]
	pcs_b_outer = [pcs_x1_lower, pcs_x1_upper, pcs_t_quad]
	pcs_m = [pcs_x1_lower, pcs_x1_upper, pcs_t_quad]
	...
	num_moments = len(generate_unsorted_idx_list(M=14,d=3))
	...
	starting_state = [-ms*9.81/ks, 0., 0.]

	ws_file = 'results/SMD/time_space_SS/M120_100000000_acc5_min_deg1.pkl'
	...
	ets = escape_time_solver(d=state_dim, M=num_moments, q_alpha=q_polynomial_deg, 
		                     ...
		                     max_opt_iters=200000, acc_lookback=5, sys_params=[ks,ms,kc],
		                     solver_verbose=True, exit_time_order=1, solver='SCS', 
							 warm_start=False, warm_start_file_path=ws_file)
