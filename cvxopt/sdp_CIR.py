...
class escape_time_solver:
	...
	def solve(self):
		...
		obj = cp.Maximize((1-self.r)*self.mj[1]+self.r*self.mj[0])
	...
	def martingale_constraint_CoV(self):
		...
		for n in range(2, len(self.mj)-1):
			constraints.append(n*self.alpha*(1-self.r)*self.mj[n+1] 
							   + (2*n*self.alpha*self.r + ((n*(n-1)*self.sigma**2)/2) + n*self.beta)*self.mj[n]
							   + ((n*(n-1)*self.sigma**2*self.r + n*self.alpha*self.r**2 + n*self.beta*self.r)/(1-self.r))*self.mj[n-1]
							   + ((n*(n-1)*self.sigma**2*self.r**2)/(2*(1-self.r)**2))*self.mj[n-2]
		...
		constraints.append((self.alpha*self.r + self.beta - self.sigma**2/2)*self.mj[0]
						   + self.alpha*(1-self.r)*self.mj[1]
						   + np.log(self.y0)
		...
		constraints.append(self.mu_1 - (self.y0-self.r)/(1-self.r)
			               - self.alpha*(1-self.r)*self.mj[2]
			               - 2*self.alpha*self.r*self.mj[1]
			               - self.beta*self.mj[1]
			               - ((self.alpha*self.r**2)/(1-self.r))*self.mj[0]
			               - ((self.beta*self.r)/(1-self.r))*self.mj[0]
			               == 0)
...
if __name__ == '__main__':
	beta = [0.0, 0.2, 0.5, 1.0,2.0,5.0]#[0.5, 1.0,2.0,5.0]#[0.0, 0.2, 0.5, 1.0,2.0,5.0]
	alpha = [-1.0, -0.5, 0.0, 0.5, 1.0, 2.0, 3.0]


	for b in beta:
		ets = escape_time_solver(d=1, M=12, pcs_m=[0,1,-1], y0=0.2, beta=b, alpha=1.0)
		start_time = time.time()
		ets.solve()
		print('end time:',time.time()-start_time)

	# ets = escape_time_solver(d=1, M=12, pcs_m=[0,1,-1], y0=0.2, beta=0.5, alpha=1.0)
	# start_time = time.time()
	# ets.solve()
	# print('end time:',time.time()-start_time)