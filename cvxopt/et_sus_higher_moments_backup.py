...
if __name__ == '__main__':
	...
	# idx_list = generate_unsorted_idx_list(M=15,d=3)
	# idx_list = sorted(idx_list, key=cmp_to_key(index_comparison))
	# for i in idx_list:
	# 	print(i)
	# print(len(idx_list), '\n')
	...
	# 
	# 2) State space dimensionality: 3
	...
	# pcs_m[idx_list.index([0,0,0])] = -3.180005
	# pcs_m[idx_list.index([1,0,0])] = 3.85
	# pcs_m[idx_list.index([2,0,0])] = -1
	# pcs_m[idx_list.index([0,2,0])] = -1/9
	...
	# idx_list = sorted(idx_list, key=cmp_to_key(index_comparison))
	# pcs_m = np.zeros(len(idx_list)).tolist()
	# pcs_m[idx_list.index([0,0,0])] = -2.35
	# pcs_m[idx_list.index([1,0,0])] = 3.35
	# pcs_m[idx_list.index([2,0,0])] = -1
	# pcs_b = [[-i if i != 0 else 0 for i in pcs_m]]
	# pcs_m = [pcs_m]
	...
	# Test safe sets:
	# x1,x2 no limits, t<=0 -> -t>=0
	q_polynomial_deg = 1
	num_inequalities = 2
	idx_list = generate_unsorted_idx_list(M=q_polynomial_deg,d=state_dim)
	idx_list = sorted(idx_list, key=cmp_to_key(index_comparison))
	pcs_m1 = np.zeros(len(idx_list)).tolist()
	pcs_m1[idx_list.index([0,0,1])] = 1
	pcs_m1[idx_list.index([0,0,0])] = 2
	pcs_m2 = np.zeros(len(idx_list)).tolist()
	pcs_m2[idx_list.index([0,0,1])] = -1
	pcs_m2[idx_list.index([0,0,0])] = 5
	pcs_b = [[-i if i != 0 else 0 for i in pcs_m1]]
	pcs_m = [pcs_m1, pcs_m2]
	...
	ets = escape_time_solver(d=state_dim, M=num_moments, q_alpha=q_polynomial_deg, 
		                     ...
		                     max_opt_iters=1000, acc_lookback=0, sys_params=[ks,ms,kc],
		                     solver_verbose=True, exit_time_order=1, solver='MOSEK')

	start_time = time.time()
	ets.solve(mode='Maximize')
	print('end time:',time.time()-start_time)

	#pickle.dump(ets, open('results/SMD/time_aug/M650_10000000_acc0_min_deg2.pkl', 'wb'))
